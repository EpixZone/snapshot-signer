...
async function main() {
  ...
  // Calculate vesting start time (current time + 1 hour)
  const vestingStartTime = Math.floor(Date.now() / 1000) + 3600; // 1 hour from now
  ...
  console.log("Vesting start time:", new Date(vestingStartTime * 1000).toISOString());
  ...
  for (let i = 0; i < allocations.length; i += batchSize) {
    ...
    console.log(`Adding allocations batch ${i / batchSize + 1}/${Math.ceil(allocations.length / batchSize)}`);
  ...
  const totalAmount = allocations.reduce((sum, a) => sum + BigInt(a.amount), BigInt(0)) + 
                      BigInt(bizdevAmount) + BigInt(bizdevBonus);
  
  console.log("Total amount needed:", totalAmount.toString());
  console.log("Please send", totalAmount.toString(), "wei to the contract address:", vestingAddress);
